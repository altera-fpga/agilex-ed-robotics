...
set_shell_parameter EXTRA_MIX_INPUTS_XPRT   {0}
...
proc create_isp_subsystem {} {
    ...
    set v_extra_mix_inputs_xprt  [get_shell_parameter EXTRA_MIX_INPUTS_XPRT]
    ...
    set_instance_parameter_value      isp_vid_clk_out_bridge    EXPLICIT_CLOCK_RATE             {297000000.0}
    ...
    # isp_mixer
    # Assuming 3 is the minimal mixer num of inputs (Background, Main Stream, Icon), user defines
    # any other extra inputs to the mixer, exported to the top of the subsystem
    set_instance_parameter_value      isp_mixer                 NUM_LAYERS                      [expr {${v_extra_mix_inputs_xprt} +3} ]
    ...
    set_instance_parameter_value      isp_out_proto_conv        PIXELS_IN_PARALLEL              ${v_vid_out_if_pip}
    ...
    # To connect extra stream inputs to the mixer (exported)
    for {set in_id 0} {${in_id} < ${v_extra_mix_inputs_xprt}} {incr in_id} {
        set v_mixer_input_id [expr ${in_id} + 3]
        add_interface           exp_in_mix_vid_axis_${in_id}     axi4stream  subordinate
        set_interface_property  exp_in_mix_vid_axis_${in_id}     EXPORT_OF   isp_mixer.axi4s_vid_${v_mixer_input_id}_in
    }

    ### Assign Base Addresses ###
    set_connection_parameter_value isp_mm_bridge.m0/isp_tpg.av_mm_control_agent                 baseAddress "0x00002800"
    set_connection_parameter_value isp_mm_bridge.m0/isp_vfb.av_mm_control_agent                 baseAddress "0x00002a00"
    set_connection_parameter_value isp_mm_bridge.m0/isp_mixer.av_mm_control_agent               baseAddress "0x00002000"
    set_connection_parameter_value isp_mm_bridge.m0/isp_1d_lut.av_mm_control_agent              baseAddress "0x00000000"
    set_connection_parameter_value isp_mm_bridge.m0/vid_out_pip_conv.av_mm_control_agent        baseAddress "0x00002600"
    set_connection_parameter_value isp_mm_bridge.m0/isp_out_proto_conv.av_mm_control_agent      baseAddress "0x00002400"
    ...
    lock_avalon_base_address  isp_out_proto_conv.av_mm_control_agent
...
proc add_auto_connections {} {
    ...
    add_auto_connection   ${v_instance_name} vid_clk_out      297000000
    add_auto_connection   ${v_instance_name} vid_rst_out      297000000

    # from isp pipe
    add_auto_connection   ${v_instance_name}    vid_in              isp_out_vid_axis_0

    # To connect extra stream inputs to the mixer (exported)
    for {set in_id 0} {${in_id} < ${v_extra_mix_inputs_xprt}} {incr in_id} {
        add_auto_connection   ${v_instance_name}    exp_in_mix_vid_axis_${in_id}     to_mix_vid_axis_${in_id}
    }
    ...
    if { ${v_fpga_emif_async} } {
        ...
        add_auto_connection    ${v_instance_name}    emif_clk_in         emif_2_user_clk
    }

    # isp_emif_rst_bridge
    add_auto_connection    ${v_instance_name}    emif_rst_in         emif_2_user_rst

    # to emif s0_axi4
    add_auto_connection    ${v_instance_name}    av_mm_host_se_vfb   emif_2_user_data
